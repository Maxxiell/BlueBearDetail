/**
 * Profile photo: crop/zoom, save per signed-in user (localStorage key bbdAccountAvatar:<userId>).
 * Mount any block with [data-bbd-account-avatar] (admin dashboard + account dashboard).
 */
import { supabase } from "./auth-client.js";
import { isSupabaseConfigured } from "./supabase-config.js";
import { LEGACY_AVATAR_KEY, getAvatarStorageKey } from "./avatar-storage.js";
...
const avatarRefreshers = new Set();
let authListenerBound = false;

function bindAuthAvatarRefresh() {
  if (!isSupabaseConfigured() || authListenerBound) return;
  authListenerBound = true;
  supabase.auth.onAuthStateChange(function () {
    avatarRefreshers.forEach(function (fn) {
      try {
        fn();
      } catch (_e) {}
    });
...
/**
 * One-time: move old shared key to the current user key, then delete the legacy key.
 */
async function migrateLegacyAvatarBlob() {
  const key = await getAvatarStorageKey();
  try {
    const legacy = store.getItem(LEGACY_AVATAR_KEY);
    if (!legacy || !/^data:image\//i.test(legacy)) return;
    if (!store.getItem(key)) store.setItem(key, legacy);
    store.removeItem(LEGACY_AVATAR_KEY);
  } catch (_e) {}
}
...
function resolveAvatarUrl(custom) {
  if (isAvatarUrl(custom)) return custom.trim();
  return DEFAULT_AVATAR_URL;
}

async function safeRead() {
  await migrateLegacyAvatarBlob();
  const key = await getAvatarStorageKey();
...
function mountAvatarEditor(root) {
  ...
  async function loadFromStorage() {
    pendingDataUrl = null;
    const custom = await safeRead();
    showPreview(resolveAvatarUrl(custom));
  }

  const refresh = function () {
    loadFromStorage().catch(function () {});
  };
  avatarRefreshers.add(refresh);
  bindAuthAvatarRefresh();
  ...
  saveBtn.addEventListener("click", function () {
    const run = async function () {
      const url = pendingDataUrl != null ? pendingDataUrl : await safeRead();
      if (!url || !/^data:image\//i.test(url)) {
        toast("Choose and adjust an image first.", "info");
        return;
      }
      if (await safeWrite(url)) {
        pendingDataUrl = null;
